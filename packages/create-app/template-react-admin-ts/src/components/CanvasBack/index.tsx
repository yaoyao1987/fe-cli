...
export default function CanvasBack(props: Props): JSX.Element {
  ...
  const init = (row: number, col: number, width: number, height: number): void => {
    const stepRow: number = height / (row - 2)
    const stepCol: number = width / (col - 2)
    for (let i = 0; i < row; i++) {
      for (let j = 0; j < col; j++) {
        const temp: Dot = {
          x: j * stepCol - stepCol / 2, // 原始坐标x
          y: i * stepRow - stepRow / 2, // 原始坐标y
          sx: random(-stepRow / 2, stepRow / 2), // 当前偏移量x
          sy: random(-stepCol / 2, stepCol / 2), // 当前偏移量y
  ...
  const animate = (): void => {
    const { row } = props
    const { col } = props
    const { width } = data.current
    const { height } = data.current
    const stepRow = height / (row - 2)
    const stepCol = width / (col - 2)

    data.current.dots.forEach(function (item, index) {
      if (item.dx) {
        // 增
        if (item.sx < stepCol / 3) {
          item.sx += 0.1
        } else {
          item.dx = !item.dx
        }
    ...
    draw(data.current.dots, row, col, data.current.ctx as CanvasRenderingContext2D, width, height)
